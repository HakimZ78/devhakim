-- ===================================
-- CREATE MISSING TABLES FOR PORTFOLIO (FIXED VERSION)
-- ===================================
...
-- Insert default skill categories (only if table is empty) - FIXED VERSION
...
) AS v(title, skills, color, icon_name, order_index)
WHERE NOT EXISTS (SELECT 1 FROM skill_categories);

-- Insert default skill focus (only if table is empty) - FIXED VERSION
...
SELECT * FROM (VALUES 
  ...
  ('DevOps/Docker', 60, 5)
) AS v(skill, progress, order_index)
...
-- Insert sample milestones/timeline events (only if table is empty) - FIXED VERSION
INSERT INTO milestones (
  title, description, target_date, category, details, status, order_index
)
SELECT * FROM (VALUES 
  ('Started Healthcare Career', 'Qualified as Pharmacist and Optometrist, beginning professional healthcare journey', 'July 2010', 'education', ARRAY['Completed Pharmacy degree', 'Qualified as Optometrist', 'Started working in healthcare sector'], 'completed', 1),
  ('Discovered Trading', 'Started retail forex trading, sparking interest in financial markets and data analysis', 'March 2018', 'milestone', ARRAY['Began learning technical analysis', 'Started live trading account', 'Discovered passion for data-driven decisions'], 'completed', 2),
  ('First Coding Experience', 'Started learning Python for trading automation and data analysis', 'January 2023', 'learning', ARRAY['Completed Python basics course', 'Built first trading scripts', 'Discovered love for programming'], 'completed', 3),
  ('Built ForexAcuity', 'Developed comprehensive forex analytics platform with real-time data and pattern recognition', 'December 2024', 'project', ARRAY['Learned Next.js and React', 'Implemented WebSocket architecture', 'Launched with paying customers'], 'completed', 4),
  ('Career Transition Goal', 'Secure first software engineering role, ideally in fintech or healthcare tech', 'June 2025', 'goal', ARRAY['Complete portfolio with admin system', 'Apply to target companies', 'Prepare for technical interviews'], 'in-progress', 5)
) AS v(title, description, target_date, category, details, status, order_index)
WHERE NOT EXISTS (SELECT 1 FROM milestones);

-- Insert sample code templates (only if very few exist) - FIXED VERSION
INSERT INTO code_templates (title, description, category, language, filename, content, tags, featured, order_index)
SELECT * FROM (VALUES 
  ('Express.js API Server', 'Basic Express.js server setup with middleware, routes, and error handling', 'server', 'javascript', 'server.js', 'const express = require(''express'');
...
});', ARRAY['express', 'server', 'api', 'middleware'], true, 10),
  ('Next.js API Route', 'Template for Next.js API routes with error handling and TypeScript', 'api', 'typescript', 'route.ts', 'import { NextRequest, NextResponse } from ''next/server'';
...
}', ARRAY['nextjs', 'api', 'typescript', 'route'], true, 11)
) AS v(title, description, category, language, filename, content, tags, featured, order_index)
